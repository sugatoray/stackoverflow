...
> ⭐⭐⭐ Jump to **section `H`** to see the code used to make the plots.
...
## F. Calculate `True` Cluster Centers
...
## G. Define, Fit and Predict using `KMeans` Model
...
### G.1. Predict for `y_train` using `X_train`
...
### G.2. Predict for `y_test` using `X_test`
...
## H. Make Figure with `train`, `test` and `prediction` data
